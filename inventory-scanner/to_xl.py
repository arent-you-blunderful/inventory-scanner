...
import xlsxwriter
import time
from _settings import CUSTOMERS
...
def format_excel_table(writer, table_coords, title_coords):
    ...
    worksheet.set_column('A:A', 7, num_fmt)
    worksheet.set_column('B:B', 25, num_fmt)
    worksheet.set_column('C:C', 14, gtin_fmt)
    worksheet.set_column('D:D', 32, num_fmt)
    worksheet.set_column('E:E', 8, float_fmt)
    for title in title_coords:
        worksheet.write(title_coords[title],0, 'CUSTOMER: %s'%pretty_customers(title), title_fmt)
...
def mk_file():
    ...
    # return os.path.abspath('D:\\Dropbox\\mm_data\\{}'.format(filename))
    return os.path.abspath('C:\\Users\\User\\Dropbox\\mm_data\\{}'.format(filename))
